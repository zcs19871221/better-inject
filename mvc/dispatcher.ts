...
export default class Dispatch {
  ...
  async doDispatch(request: IncomingMessage, response: ServerResponse) {
    ...
    try {
      ...
      modelView = await this.processExcetion(error as any, webRequest, handler);
    }
    if (modelView === null) {
      return;
    }
  ...
  private async render(modelView: ModelView, webRequest: WebRequest) {
    ...
    if (modelView.hasView()) {
      template = await readFile(modelView.getView(), 'utf-8');
    } else {
      template = `p #{message}\np #{stack}`;
    }
    if (!webRequest.getRequestHeader('content-type')) {

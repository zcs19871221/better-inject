...
function getUrlpics() {
  ...
  if (PicUrls.indexOf('mh160tuku') == -1) PicUrls = base64_decode(picTree);
...
function getcurpic(v) {
  ...
  if (v.indexOf('qq.com/store_file_download') != -1) {
    ...
    s =
      'http://img8.hgysxz.cn/picinc/qTcms.Pic.FangDao.asp?p=' +
      base64_encode(v);
...
const getPics = () => {
  return getUrlpics().map(getcurpic);
};

export const getPics = getPics;
export const setParams = (pics, id) => {
  picTree = pics;
  currentChapterid = id;
};
